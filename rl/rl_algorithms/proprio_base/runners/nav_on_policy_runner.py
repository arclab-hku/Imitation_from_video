...
class ProprioBaseNavOnPolicyRunner:
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    obs_dict, rewards, dones, infos, reset_env_ids = self.env.step(actions)

                    obs, critic_obs, rewards, dones = obs_dict['obs'].to(self.device),\
                                                      obs_dict['privileged_obs'].to(self.device), \
    ...
    def restore_train(self, path):  # fine tune
        ...
        cprint(f'[Fine Tune] Loading pre-trained navigation policy from: {path}', 'green', attrs=['bold'])
        loaded_dict = torch.load(path)
